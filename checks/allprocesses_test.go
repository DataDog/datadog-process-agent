...
var (
	...
	teststring2 = ".NET CLR-säkerhet ! Microsoft.Exchange.UM.CallRouter ! Tid för körningskontroller i procent"
	testbuffer2 = []uint16{
...
func TestWindowsStringConversion(t *testing.T) {
	converted := convertWindowsString(testbuffer1)
	assert.Equal(t, teststring1, converted)

	converted = convertWindowsString(testbuffer2)
	assert.Equal(t, teststring2, converted)
}
